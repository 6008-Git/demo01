...
//按钮对应方法生成
function oldBtnMethodsTrans(oldBtn){//生成含有注释的方法
//去掉所有制表符
    oldBtn = oldBtn.replace(/\t/g,'')
//console.log(oldBtn)
...
//遍历数组，提取内容，得到新的方法与对应button标签
    for (let i = 0; i < oldBtn_Arr.length; i++) {
        ...
        if(oldForm_split[0]==='<dw:button'){
            ...
            newBtn = newBtn1+methodsName+newBtn2+btnName+newBtn3
            //console.log(newBtn)
            var comments = '//'+btnName+'方法'+'\n'
            var method1 = methodsName+'{\n'
            var method2 = ''+'\n'
            var method3 = '}'+','
            var allMethod = comments+method1+method2+method3
            //console.log(allMethod)
            allMethodSum = allMethodSum+allMethod+'\n'
        }
    }
    return allMethodSum
}